...
class CommunicationStateMachine(secsgem.common.StateMachine):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, settings: secsgem.common.Settings) -> None:
        ...
        self.disabled = secsgem.common.State(CommunicationState.DISABLED, "DISABLED")
        self.enabled = secsgem.common.State(CommunicationState.ENABLED, "ENABLED")
        self.not_communicating = secsgem.common.State(
            CommunicationState.NOT_COMMUNICATING, "NOT_COMMUNICATING", parent=self.enabled
        )
        self.host_initiated_connect = secsgem.common.State(
            CommunicationState.HOST_INITIATED_CONNECT, "HOST_INITIATED_CONNECT", parent=self.enabled
        )
        self.wait_cr_from_host = secsgem.common.State(
            CommunicationState.WAIT_CR_FROM_HOST, "WAIT_CR_FROM_HOST", parent=self.enabled
        )
        self.equipment_initiated_connect = secsgem.common.State(
            CommunicationState.EQUIPMENT_INITIATED_CONNECT, "EQUIPMENT_INITIATED_CONNECT", parent=self.enabled
        )
        ...
        self.communicating = secsgem.common.State(
            CommunicationState.COMMUNICATING, "COMMUNICATING", parent=self.enabled
        )
